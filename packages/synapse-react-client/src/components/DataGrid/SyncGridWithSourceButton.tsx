...
export default function SyncGridWithSourceButton(
  ...
  const { mutate: mergeGrid, isPending } = useMergeGridWithSource({
    onSuccess: (result, variables) => {
      ...
      } else if (result.type === 'tableUpdateTransaction') {
  ...
  // RecordSet sync supports PULL. If the RecordSet source or JSON Schema has been updated
  // since this grid was created, we have the user PULL the latest changes before enabling
  // a full PULL_PUSH, so they have an opportunity to handle unexpected merge outcomes.
  const shouldPull =
    entity !== undefined &&
...
export function getSyncButtonLabels(
  ...
  if (sourceEntityType === EntityType.table) {
    return {
      ...
      tooltipText:
        'Applies any changes made in this Curator session to the source table',
    }
  }
  return {
